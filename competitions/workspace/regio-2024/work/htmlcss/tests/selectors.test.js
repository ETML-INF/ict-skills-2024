...
const initSelectors = (s) => {
  ...
  selectors = selectors.filter((rule) => {
    const forbidden = [
      "nth-child",
      "nth-last-child",
      "nth-of-type",
...
// selector getter
const getSelector = (taskId) =>
  selectors.find((rule) => rule.startsWith("#task-" + taskId));
const getOriginalSelector = (taskId) =>
  originalSelectors.find((rule) => rule.startsWith("#task-" + taskId));
...
describe("CSS Selectors", function () {
  before(async () => {
    try {
      ...
      const htmlData = await fetch("/base/src/task.html")
        .then((response) =>
          response.status === 404 ? fetch("src/task.html") : response
        )
      ...
      const css = await fetch("/base/src/task.css")
        .then((response) =>
          response.status === 404 ? fetch("src/task.css") : response
        )
        .then((response) => response.text());
  ...
  for (let i = 1; i <= 10; i++) {
    it("Task " + i, () => {
      // has selector
      expect(getSelector(i), `Selector "${getOriginalSelector(i)}" not allowed`)
        .not.to.be.undefined;
      ...
      [...html.querySelectorAll(`#task-${i} [data-target]`)].forEach((item) =>
