...
class TestRun {
  ...
  recordTest(fullName, testName, extra, successful) {
    if (extra) {
  ...
  getWarnings() {
    const warnings = []; // search for groups without any recorded test

    const emptyGroups = this.getGroups().filter(group => !group.hasTests());

    if (this.extraTestRecorded && emptyGroups.length > 0) {
      warnings.push(`The following groups do not have any test:\n${emptyGroups.map(group => `  - ${group.getDisplayName()}`).join('\n')}`);
    } // search for tests that do not have extra tests


    const missingExtraTests = this.getGroups().map(group => group.getMissingExtraTests().map(test => `  - ${group.getDisplayName()} > ${test.getName()}`)).filter(tests => tests.length > 0);

    if (this.extraTestRecorded && missingExtraTests.length > 0) {
      warnings.push(`The following tests do NOT have extra tests and so can NOT be checked for possible cheating:\n${missingExtraTests.map(tests => tests.join('\n')).join('\n')}`);
    } // search for extra tests that do not have a normal test


    const missingNormalTests = this.getGroups().map(group => group.getMissingNormalTests().map(test => `  - ${group.getDisplayName()} > ${test}`)).filter(tests => tests.length > 0);

    if (missingNormalTests.length > 0) {
      warnings.push(`The following extra tests do not belong to a main test and were ignored:\n${missingNormalTests.map(tests => tests.join('\n')).join('\n')}`);
    } // search for tests that do not belong to any group


    if (this.ungroupedTests.length > 0) {
      warnings.push(`The following tests do not belong to a group and were ignored:\n${this.ungroupedTests.map(test => `  - ${test}`).join('\n')}`);
    }
